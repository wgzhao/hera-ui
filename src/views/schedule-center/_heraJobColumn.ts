...
const baseColumn: PlusFormGroupRow[] = [
  {
    ...
    columns:[
  { label: "任务id", prop: "id"},
  { label: "调度类型", prop: "scheduleType", valueType: "select", options: [
    {label: "定时调度", value: 0},
    {label:"依赖调度", value: 1}]},
    { label: "所有人", prop: "owner"},
      { label: "名称", prop: "name"},
        ...
        { label: "管理员", prop: "uIdS"},
        { label: "自动调度", prop: "auto", valueType: "switch"},
  ...
  },
    {
     ...
     {label: "配置项信息", prop: "selfConfigs", colProps: {span: 24}},

      { label: "继承的配置项信息", prop: "inheritConfig", colProps: {span: 24}}
      ]
    },
    {
      title:"脚本",
      columns: [
         { label: "脚本", prop: "script", valueType: "area",
         },
      ]
    },
]
...
cronColumn[0].columns.splice(4, 0, { label: "定时表达式", prop: "cronExpression"});

// 依赖任务字段
const depColumn: PlusFormGroupRow[] = deepCopy(baseColumn);
depColumn[0].columns.splice(4, 0, { label: "依赖任务", prop: "dependencies"});
depColumn[0].columns.splice(7, 0, { label: "依赖周期", prop: "heraDependencyCycle"})
